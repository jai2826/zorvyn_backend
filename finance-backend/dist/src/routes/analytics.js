...
export const analyticsRoutes = new Hono()
    .get("/summary", authenticate, authorize(["ANALYST", "ADMIN"]), async (c) => {
    try {
        ...
        // Calculate Totals
        const totalIncome = Number(stats.find((s) => s.type === "INCOME")?._sum
            .amount || 0);
        const totalExpense = Number(stats.find((s) => s.type === "EXPENSE")?._sum
            .amount || 0);
        return c.json({
            totalIncome,
            totalExpense,
        ...
        return c.json({ error: "Analytics calculation failed" }, 500);
    }
})
