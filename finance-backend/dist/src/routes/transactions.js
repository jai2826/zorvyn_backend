...
export const transactionRoutes = new Hono()
    ...
    // 2. CREATE (Admin Only)
    .post("/create/:userId", authenticate, authorize(["ADMIN"]), zValidator("json", createSchema), async (c) => {
    ...
    if (user?.isActive === false) {
        return c.json({
            error: "Cannot add transactions to a deactivated account. First activate the account.",
        }, 403);
    }
    try {
        ...
        console.error(error);
        return c.json({ error: "Could not create transaction" }, 500);
    }
})
    // 3. UPDATE (Admin Only)
    .patch("/:id", authenticate, authorize(["ADMIN"]), zValidator("json", createSchema.partial()), async (c) => {
    ...
    // 4. DELETE (Admin Only)
    .delete("delete/:id", authenticate, authorize(["ADMIN"]), async (c) => {
    const { id } = c.req.param();
    if (!id) {
